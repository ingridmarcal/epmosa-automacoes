...
output_file="/Users/imarcal/Documents/Unesp/Experiments/PrimeiraGeracao/gen_times_compress_sc.txt"
...
for ((i=0; i<${#identifiers[@]}; i++)); do
    ...
    for ((nb=1; nb<=${n_bugs[$i]}; nb++)); do
        ...
        for nv in "${n_versions[@]}"; do
            ...
            perl "/Users/imarcal/Documents/Unesp/defects4j/framework/bin/gen_tests.pl" -g evosuite -p "${identifiers[$i]}" -v "${nb}b" -n "${nv}" -o ./evosuite_tests_sc -b 300
			end_time_gen=$(date +%s)
